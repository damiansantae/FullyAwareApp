...
public class SelectectDeselectToDoActionTest {

    @Rule
    public ActivityTestRule<ListToDoViewMasterTesting> mActivityTestRule = new ActivityTestRule<>(ListToDoViewMasterTesting.class);

    @Test
    public void selectectDeselectToDoActionTest() {
        ViewInteraction appCompatTextView = onView(
                allOf(withId(R.id.title), withText("Titulo1"), isDisplayed()));
        appCompatTextView.perform(longClick());

        ViewInteraction imageButton = onView(
                allOf(withId(R.id.floatingDoneButton),
                        childAtPosition(
                                allOf(withId(R.id.content_listToDo),
                                        childAtPosition(
        ...
        ViewInteraction imageButton2 = onView(
                allOf(withId(R.id.floatingDeleteButton),
                        childAtPosition(
                                allOf(withId(R.id.content_listToDo),
                                        childAtPosition(
                                                IsInstanceOf.<View>instanceOf(android.view.ViewGroup.class),
